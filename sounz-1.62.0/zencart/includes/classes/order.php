 ...
 * @version $Id: order.php 6534 2007-06-26 01:01:00Z drbyte $
 */
/**
 * order class
 *
...
$store_country = $db->Execute("SELECT configuration_value FROM zencartconfiguration WHERE configuration_key = 'STORE_COUNTRY'");
define('STORE_COUNTRY', $store_country->fields['configuration_value']);

class order extends base {
  var $info, $totals, $products, $customer, $delivery, $content_type, $email_low_stock, $products_ordered_attributes,
  $products_ordered, $products_ordered_email, $special_library_order;
  ...
  function order($order_id = '') {
    ...
    $this->special_library_order = false;
    ...
    if ($this->delivery['country_id'] != STORE_COUNTRY) {
      ...
      for ($i=0, $n=sizeof($products); $i<$n; $i++) {
        ...
        if (($products[$i]['products_class'] == 'LOAN_MANIFESTATION') || ($products[$i]['products_class'] == 'LOAN_RESOURCE')) {
          $this->special_library_order = true;
        }
  ...
  function cart() {
    ...
    $customer_address_query = "select c.customers_firstname, c.customers_lastname, c.customers_telephone,
                                   ...
                                   from " . TABLE_CUSTOMERS . " c left join " . TABLE_ADDRESS_BOOK . " ab on c.customers_id=ab.customers_id  
...
*/
  ...
  function create($zf_ot_modules, $zf_mode = 2) {
	...
	if ($this->customer['format_id'] == '')
		{$this->customer['format_id'] = 0;}
    if ($this->delivery['format_id'] == '')
		{$this->delivery['format_id'] = 0;}
    if ($this->billing['format_id'] == '')
		{$this->billing['format_id'] = 0;}
...
#echo "HELLO ".$this->customer['format_id'];
#echo "HELLO ".$this->delivery['format_id'];
#echo "HELLO ".$this->billing['format_id'];
    ...
    $insert_id = $db->Insert_ID(TABLE_ORDERS_SEQ);
    ...
    for ($i=0, $n=sizeof($this->products); $i<$n; $i++) {
      ...
      if (STOCK_LIMITED == 'true') {
        if (DOWNLOAD_ENABLED == 'true') {
          $stock_query_raw = "select p.products_class,p.manifestation_id,p.products_quantity, pad.products_attributes_filename, p.product_is_always_free_shipping
        ...
        if ($stock_values->RecordCount() > 0) {
		  ...
		  #UPDATED BY PETE TO SUPRESS ERROR	
		  $sounz_product_id=zen_get_prid($this->products[$i]['id']);
			
		  #note no database activity needed for donations currently - may implement easily-sounz-accessible transaction log.  
		
		  if ($stock_values->fields['products_class']=='SERVICE')
			{
			#find our login id
			$customer_id=(int)$_SESSION['customer_id'];
			
			#this is a membership product - we need to update our users member types
			
			#if we have specific IDs defined, we should use them, otherwise find the membership product id 
			
			#find our tag and associated member_type_id
			$service = $db->Execute("select zencart_tag,member_type_id,subscription_duration,subscription_item_count from ".SOUNZ_SERVICES_TABLE." s WHERE s.sounz_service_id=".$sounz_product_id." LIMIT 1");
			$tag = $service->fields['zencart_tag'];
			$member_type_id=$service->fields['member_type_id'];
		        $subscription_duration=$service->fields['subscription_duration'];
			$subscription_item_count=$service->fields['subscription_item_count'];
			
                        if ($subscription_item_count == '')
				{
				$subscription_item_count=0;
				}
			if ($tag != '' && $member_type_id != '')
				{
				#lets find our current date of membership, if any
				$service = $db->Execute("select expiry_date from memberships m WHERE m.login_id=$customer_id and m.member_type_id=$member_type_id and m.pending_payment=false order by m.expiry_date LIMIT 1");
				$current_expiry_date=$service->fields['expiry_date'];
				#if we have paid by credit card, and are here - our payment pending is false
				#echo "PAYMENT MODULE: ".$this->info['payment_module_code'];
				$pending_payment='true';
				if 	($this->info['payment_module_code']=='dpsaccess')
					{
				...
				if ($current_expiry_date != '')
					{
					$db->Execute("insert into memberships (login_id,member_type_id,expiry_date,renewed_date,pending_payment,loan_count, sounz_service_id) values (".$customer_id.",".$member_type_id.",timestamp '".$current_expiry_date."'+interval '".$subscription_duration."',now(),$pending_payment,$subscription_item_count, $sounz_product_id);");
					}
				else
					{
					$db->Execute("insert into memberships (login_id,member_type_id,expiry_date,purchased_date,pending_payment,loan_count, sounz_service_id) values (".$customer_id.",".$member_type_id.",now()+interval '".$subscription_duration."',now(),$pending_payment,$subscription_item_count, $sounz_product_id);");
					}
				}
			else
				{
			...
			else if ($stock_values->fields['products_class']=='LOAN_MANIFESTATION')
			{
		    #is the item a loan item?
		    #if so, we need to update the appropriate item's out on loan status, and update a table so we know who it is on loan to.
	        #find the first loanable item:
			
			$manifestation_id=$stock_values->fields['manifestation_id'];
			
		    #echo "ORDER MANIFESATION ID: $manifestation_id\n";
			$updateItem = $db->Execute("select item_id,out_on_loan_or_hire from ".SOUNZ_ITEM_TABLE." i left join item_types it on i.item_type_id=it.item_type_id where i.manifestation_id=".$manifestation_id." and it.item_type_desc='Music library item' limit 1");
			$item_id = $updateItem->fields['item_id'];
			$loan_status = $updateItem->fields['out_on_loan_or_hire'];
			#echo "select item_id from ".SOUNZ_ITEM_TABLE." i left join item_types it on i.item_type_id=it.item_type_id where i.manifestation_id=".$manifestation_id." and it.item_type_desc='Music library item' limit 1";
			#this should never be the case, but don't update the item as being on loan if it is cloneable
			if (($this->products[$i]['products_virtual'])==0)
				{
                # SOUNZ wants manually assign appropriate copies of LOAN items
                # hence, we only reserve an item (so that appropriate Manifestation
                # can be tracked) without setting it to out_on_loan_or_hire here - WR#51184

				#if ($loan_status == 'f')
				#	{     
				#	$db->Execute("update items set out_on_loan_or_hire=true where item_id=$item_id");
				#	
				#	$db->Execute("insert into ".SOUNZ_BORROWED_ITEMS_TABLE." (item_id,hired_out,date_borrowed,login_id,date_due,created_at,updated_at,active) values ( $item_id, false,now(),".$_SESSION['customer_id'].",(now()+interval '".SOUNZ_LOAN_PERIOD."' ),now(),now(),true)");
				#	}
				#else
				#	{
                                        #standard loan period is 60d, set in configure.php
					$db->Execute("insert into ".SOUNZ_BORROWED_ITEMS_TABLE." (item_id,hired_out,date_borrowed,login_id,date_due,created_at,updated_at,active,reserved) values ( $item_id, false,now(),".$_SESSION['customer_id'].",(now()+interval '".SOUNZ_LOAN_PERIOD."' ),now(),now(),true,true)");
				#	}
				}
		   #find our users membership to decrement the item count
		    

                   $membership = $db->Execute("SELECT membership_id,loan_count 
                                   FROM memberships m LEFT JOIN logins l ON m.login_id=l.login_id
                                      LEFT JOIN ".SOUNZ_SERVICES_TABLE." ss ON m.sounz_service_id = ss.sounz_service_id " 
                                      #LEFT JOIN member_types mt ON m.member_type_id=mt.member_type_id 
                                      # WHERE mt.member_type_desc='Library Membership'
                                     ." WHERE ss.sounz_service_name ilike '%standard library membership%' 
                                    AND l.login_id=".$_SESSION['customer_id'] ." AND pending_payment=false LIMIT 1;");

		    $membership_id = $membership->fields['membership_id'];
			$loan_count = $membership->fields['loan_count'];
		    if ($membership_id != '')
				{
				$db->Execute("update memberships set loan_count=".($loan_count-1)." where membership_id=$membership_id;");
				}
					
				
			}
		  
		else if ($stock_values->fields['products_class']=='LOAN_RESOURCE')
			{
			
			$resource_id=$stock_values->fields['manifestation_id'];
			
		    #echo "ORDER RESOURCE ID: $resource_id\n";
			$updateItem = $db->Execute("select item_id,out_on_loan_or_hire from ".SOUNZ_ITEM_TABLE." i left join item_types it on i.item_type_id=it.item_type_id where i.resource_id=".$resource_id." and it.item_type_desc='Resource library item' limit 1");
			$item_id = $updateItem->fields['item_id'];
			$loan_status = $updateItem->fields['out_on_loan_or_hire'];
			#echo "select item_id from ".SOUNZ_ITEM_TABLE." i left join item_types it on i.item_type_id=it.item_type_id where i.resource_id=".$resource_id." and it.item_type_desc='Resource library item' limit 1";
			#this should never be the case, but don't update the item as being on loan if it is cloneable
			if (($this->products[$i]['products_virtual'])==0)
				{
                # SOUNZ wants manually assign appropriate copies of LOAN items
                # hence, we only reserve an item (so that appropriate Resource
                # can be tracked) without setting it to out_on_loan_or_hire here - WR#51184
		...
		else if ($stock_values->fields['products_class']=='SALE_MANIFESTATION')
			{
		  #else, we just need to delete an item that matches the manifestation id and is a sale item;
		  #loop for number of items in cart
			#find our deleteable item:
			$manifestation_id=$stock_values->fields['manifestation_id'];
			
			$deleteItem=$db->Execute("select item_id from ".SOUNZ_ITEM_TABLE." i left join item_types it on i.item_type_id=it.item_type_id where i.manifestation_id=".$manifestation_id." and it.item_type_desc='Sale item' limit 1");
			
			$item_id = $deleteItem->fields['item_id'];
			#don't delete the item if the products is virtual.
			if ($this->products[$i]['products_virtual']==0 && $item_id != '')
				{
				$db->Execute("delete from " . SOUNZ_ITEM_TABLE ." where item_id=$item_id" );
				}
			#we need to generate a sale tranaction moneyworks file
			
			#we need a moneyworks transaction folder config setting
			$export=fopen(MONEYWORKS_EXPORT_FOLDER."/Transaction_".strftime("%Y-%m-%dT%T",time()).".csv","w");
			fwrite($export,implode(",",$sql_data_array)."\n");
			fclose($export);
			
			}

		else if ($stock_values->fields['products_class']=='SALE_RESOURCE')
			{
		  #else, we just need to delete an item that matches the manifestation id and is a sale item;
		  #loop for number of items in cart
			#find our deleteable item:
			...
			$deleteItem=$db->Execute("select item_id from ".SOUNZ_ITEM_TABLE." i left join item_types it on i.item_type_id=it.item_type_id where i.resource_id=".$resource_id." and it.item_type_desc='Sale item' limit 1");
			...
			{
			#this is a donation, or something unexpected. do nothing.
      ...
      #UPDATED BY PETE TO SUPPRESS ERROR
      #$db->Execute("update " . TABLE_PRODUCTS . " set products_ordered = products_ordered + " . sprintf('%f', $this->products[$i]['qty']) . " where products_id = '" . 
	  #zen_get_prid($this->products[$i]['id']) . "'");

      $zco_notifier->notify('NOTIFY_ORDER_PROCESSING_STOCK_DECREMENT_END');

      # fix for loans to avoid breakage due to mismatch between string and numeric types
      if (!((double)$this->products[$i]['price'])) {
        $this->products[$i]['price'] = 0;
      }
      
      if (!((double)$this->products[$i]['final_price'])) {
        $this->products[$i]['final_price'] = 0;
      }
      ...
      $order_products_id = $db->Insert_ID(TABLE_ORDERS_PRODUCTS_SEQ);
      ...
      if (isset($this->products[$i]['attributes'])) {
        ...
        for ($j=0, $n2=sizeof($this->products[$i]['attributes']); $j<$n2; $j++) {
          	#echo "DOWNLOAD ENABLED: ".DOWNLOAD_ENABLED."!";
			if (DOWNLOAD_ENABLED == 'true') {
            $attributes_query = "select popt.products_options_name, poval.products_options_values_name,
                                 pa.options_values_price, pa.price_prefix,
                                 ...
                                 pad.products_attributes_maxdays, pad.products_attributes_maxcount, pad.products_attributes_filename,pad.products_attributes_filename_2,
            ...
            TABLE_PRODUCTS_ATTRIBUTES . " pa
                                  left join " . TABLE_PRODUCTS_ATTRIBUTES_DOWNLOAD . " pad
                                  on pa.products_attributes_id=pad.products_attributes_id
                                 where pa.products_id = '" . zen_db_input((int)$this->products[$i]['id']) . "'
          ...
          //                                 'products_options_values' => $attributes_values->fields['products_options_values_name'],
                                  ...
                                  'options_values_price' => $attributes_values->fields['options_values_price']+0,
                                  'price_prefix' => $attributes_values->fields['price_prefix'],
                                  'product_attribute_is_free' => $attributes_values->fields['product_attribute_is_free']+0,
                                  'products_attributes_weight' => $attributes_values->fields['products_attributes_weight']+0,
                                  'products_attributes_weight_prefix' => $attributes_values->fields['products_attributes_weight_prefix'],
                                  'attributes_discounted' => $attributes_values->fields['attributes_discounted']+0,
                                  'attributes_price_base_included' => $attributes_values->fields['attributes_price_base_included']+0,
                                  'attributes_price_onetime' => $attributes_values->fields['attributes_price_onetime']+0,
                                  'attributes_price_factor' => $attributes_values->fields['attributes_price_factor']+0,
                                  'attributes_price_factor_offset' => $attributes_values->fields['attributes_price_factor_offset']+0,
                                  'attributes_price_factor_onetime' => $attributes_values->fields['attributes_price_factor_onetime']+0,
                                  'attributes_price_factor_onetime_offset' => $attributes_values->fields['attributes_price_factor_onetime_offset']+0,
                                  ...
                                  'attributes_price_words' => $attributes_values->fields['attributes_price_words']+0,
                                  'attributes_price_words_free' => $attributes_values->fields['attributes_price_words_free']+0,
                                  'attributes_price_letters' => $attributes_values->fields['attributes_price_letters']+0,
                                  'attributes_price_letters_free' => $attributes_values->fields['attributes_price_letters_free']+0,
                                  'products_options_id' => (int)$this->products[$i]['attributes'][$j]['option_id']+0,
                                  'products_options_values_id' => (int)$this->products[$i]['attributes'][$j]['value_id']+0,
                                  'products_prid' => $this->products[$i]['id']
                                  );
          ...
          if ((DOWNLOAD_ENABLED == 'true') && isset($attributes_values->fields['products_attributes_filename']) && zen_not_null($attributes_values->fields['products_attributes_filename'])) {
            $sql_data_array = array('orders_id' => $zf_insert_id,
									...
									'orders_products_filename_2' => $attributes_values->fields['products_attributes_filename_2'],
  ...
  function send_order_email($zf_insert_id, $zf_mode) {
    ...
    if (DOWNLOAD_ENABLED == 'true')
	{
	$html_msg['DOWNLOAD_PREAMBLE'] = EMAIL_DOWNLOAD_PREAMBLE;
	}
    ...
    if (defined('SPECIAL_ORDER_NOTE')) { $email_order .= "\n".SPECIAL_ORDER_NOTE."\n"; }
    ...
    if (DOWNLOAD_ENABLED == 'true')	{ $html_msg['DOWNLOAD_PREAMBLE'] = EMAIL_DOWNLOAD_POST; }


    // include disclaimer
    $email_order .= "\n-----\n" . sprintf(EMAIL_DISCLAIMER, STORE_OWNER_EMAIL_ADDRESS) . "\n\n";

    // WR#52087 include SOUNZ signature
    $email_order .= EMAIL_STORE_SIGNATURE. "\n\n";
