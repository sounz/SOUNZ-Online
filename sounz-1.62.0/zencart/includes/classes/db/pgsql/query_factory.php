<?php
/**
 * PgSQL query_factory Class.
 * Class used for database abstraction to PgSQL
 *
 * @package classes
...
class queryFactory extends base {
  ...
  function connect($zf_host, $zf_user, $zf_password, $zf_database, $zf_pconnect = 'false', $zp_real = false)
    ...
    if (!function_exists('pg_connect')) die ('Call to undefined function: pg_connect().  Please install the PgSQL Connector for PHP');

	if ($zf_pconnect != 'false')
		{
		if ($zf_password=='')
			{$this->link = @pg_connect("host=$zf_host user=$zf_user dbname=$zf_database");}
		else
			{$this->link = @pg_connect("host=$zf_host user=$zf_user password=$zf_password dbname=$zf_database");}
    ...
    if ($this->link)
		{
      	if (@pg_dbname($this->link))
			...
			{
        	$this->set_error(0,pg_last_error($this->link), $zp_real);
        	return false;
      		}
    	}
	else
  ...
  function show_error() {
    ...
    // condition added by Liuba to avoid database errors to be displayed to users on production
    if (defined('DB_ERROR_REPORTING') && DB_ERROR_REPORTING == 'true') {
  ...
  function Execute($zf_sql, $zf_limit = false, $zf_cache = false, $zf_cachetime=0) {
    ...
    if ( $zf_cache AND $zc_cache->sql_cache_exists($zf_sql) AND !$zc_cache->sql_cache_is_expired($zf_sql, $zf_cachetime) ) {
      ...
      if (!$zp_db_resource) $this->set_error(0,@pg_last_error($this->link));
      ...
      if ($obj->RecordCount() > 0) {
        ...
        $zp_ii = 0;
	while (!$obj->EOF) {
          $zp_result_array = @pg_fetch_array($zp_db_resource);
          if ($zp_result_array) {
...
function insert_ID($sequence_name)
   {
   $result=pg_exec("SELECT last_value FROM $sequence_name");
  ...
  function metaColumns($zp_table) {
    $res = @pg_query("select * from " . $zp_table . " limit 1", $this->link);
...
class queryFactoryResult {
  ...
  function MoveNext() {
    ...
    if ($this->is_cached) {
      ...
      $zp_result_array = @pg_fetch_array($this->resource);
      if (!$zp_result_array) {
        $this->EOF = true;
      } else {
        while (list($key, $value) = each($zp_result_array)) {
  ...
  function Move($zp_row) {
    ...
    if (@pg_result_seek($this->resource, $zp_row)) {
      ...
      @pg_result_seek($this->resource, $zp_row);
      $this->EOF = false;
      return;
    } else {
      $this->EOF = true;
      $db->set_error(0,pg_last_error());
    }
  }
}
...
class queryFactoryMeta {
  ...
  function queryFactoryMeta($zp_field, $zp_res) {
    ...
    $this->max_length = @pg_field_size($zp_res, $zp_field);
  }
}
?>
