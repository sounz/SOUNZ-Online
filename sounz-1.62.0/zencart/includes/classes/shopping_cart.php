...
class shoppingCart extends base {
  ...
  function add_cart($products_id, $qty = '1', $attributes = '', $notify = true) {
	...
	$cart_max = zen_get_products_quantity_order_max($products_id);
	if ($qty > $cart_max && $cart_max > 0)
		{
		$qty=$cart_max;
		}
...
// adjust products price
  ...
  if ($check_attribute == '') {$check_attribute=0;}
  $sql = "select *
...
//  echo 'I SEE ' . $this->total . ' vs ' . $this->free_shipping_price . ' items: ' . $this->free_shipping_item. '<br>';
  ...
  function get_products($check_for_valid_cart = false) {
    ...
    reset($this->contents);
    while (list($products_id, ) = each($this->contents)) {
      $products_query = "select p.products_class,p.products_id, p.master_categories_id, p.products_status, pd.products_name, p.products_model, p.products_image,
...
// validate cart contents for checkout
        ...
        if ($new_qty == (int)$new_qty) {
 		...
 		// check to see if this product exists - could have shifted under us due to sounz views
        if ( zen_get_prid($prid)=='' ) {
            $this->remove($products_id);
		...
		$products_array[] = array('id' => $products_id,
								  'products_class' => $products->fields['products_class'],
  ...
  function get_content_type($gv_only = 'false') {
    ...
    if ( $this->count_contents() > 0 ) {
      reset($this->contents);
      while (list($products_id, ) = each($this->contents)) {
        $free_ship_check = $db->Execute("select pa.options_values_id,p.products_class,p.products_virtual, p.products_model, p.products_price, p.product_is_always_free_shipping from " . TABLE_PRODUCTS . " p left join ".TABLE_PRODUCTS_ATTRIBUTES." pa on p.products_id = pa.products_id where p.products_id = '" . zen_get_prid($products_id) . "'");
        $virtual_check = false;
		...
		if (preg_match('/DONATION/', addslashes($free_ship_check->fields['products_class']))) {
          $gift_voucher += ($free_ship_check->fields['products_price'] + $this->attributes_price($products_id)) * $this->contents[$products_id]['qty'];
        }
		if (preg_match('/SERVICE/', addslashes($free_ship_check->fields['products_class']))) {
          $gift_voucher += ($free_ship_check->fields['products_price'] + $this->attributes_price($products_id)) * $this->contents[$products_id]['qty'];
        }
		#product has a downloadable attribute
		if ($free_ship_check->fields['options_values_id'] != '') {
          $gift_voucher += ($free_ship_check->fields['products_price'] + $this->attributes_price($products_id)) * $this->contents[$products_id]['qty'];
        }
  ...
  function in_cart_mixed_discount_quantity($products_id) {
    ...
    // if nothing is in cart return 0
    if (!is_array($this->contents)) return 0;
	if (zen_get_prid($products_id)=='') return 0;
    // check if mixed is on
...
// display message if all is good and not on shopping_cart page
  ...
  function actionAddProduct($goto, $parameters) {
    ...
    if (isset($_POST['products_id']) && is_numeric($_POST['products_id'])) {
      ...
      // verify qty to add
          $real_ids = $_POST['id'];
#die('I see Add to Cart: ' . $_POST['products_id'] . 'real id ' . $POST['id']. ":". zen_get_uprid($_POST['products_id'], $real_ids) . ' add qty: ' . $add_max . ' - cart qty: ' . $cart_qty . ' - newqty: ' . $new_qty);
