...
// WR#52087 - Foreign Library Members do pay shipping cost
// process them as a special order
  if ($order->special_library_order == true) {
      $_SESSION['shipping'] = ' ';
      if (!$_SESSION['billto']) { $_SESSION['billto'] = $_SESSION['customer_default_address_id']; }
      zen_redirect(zen_href_link(FILENAME_SPECIAL_ORDER), 'zenid=' . $session , 'SSL', false, false);    
...
// a shipping address is not needed
  ...
  // Added by Liuba (Catalyst IT) as there is no validation for empty fields in zencart except javascript
  // on address form
  // begin
  $address = zen_get_customer_address($_SESSION['customer_id'], $_SESSION['sendto']);

  // check that the required address fields are not empty  
  if (!$address['street_address'] || !$address['city'] || !$address['country_id']) {
    // redirect to shipping address form
    zen_redirect(zen_href_link(FILENAME_CHECKOUT_SHIPPING_ADDRESS));
  }
