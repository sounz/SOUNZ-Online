 ...
 * @version $Id: header_php.php 6524 2007-06-25 21:27:46Z drbyte $
 */
...
$special_library_order = $order->special_library_order;
...
for ($i=0, $n=sizeof($products); $i<$n; $i++) {
  ...
  $attrArray = false;
  $productsClass = $products[$i]['products_class'];
...
//  $showFixedQuantityAmount = $products[$i]['quantity'] . zen_draw_hidden_field('cart_quantity[]', 1);
  ...
  $productsPriceEach = $currencies->display_price($products[$i]['final_price'], zen_get_tax_rate($products[$i]['tax_class_id']), 1) . ($products[$i]['onetime_charges'] != 0 ? '<br />' . 
