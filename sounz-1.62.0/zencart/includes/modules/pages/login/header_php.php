...
if (isset($_GET['action']) && ($_GET['action'] == 'process')) {
  ...
  */
  ...
  $check_customer_query = "SELECT customers_id, customers_firstname, customers_password,
                           ...
                           WHERE customers_nick ILIKE :email";
  ...
  if (!$check_customer->RecordCount()) {
    ...
    //check if we're validating a plaintext password or a raw encrypted one
	$password_validated=false;
	if (isset($_POST['encrypted']))
		{
		$password_validated=zen_validate_encrypted_password($password,$check_customer->fields['customers_password']);
		}
	else
		{
		$password_validated=zen_validate_password($password, $check_customer->fields['customers_password']);
		}
