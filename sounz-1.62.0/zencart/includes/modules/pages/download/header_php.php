...
$sql = "SELECT date_format(o.date_purchased, '%Y-%m-%d')
          AS date_purchased_day, opd.download_maxdays, opd.download_count, opd.download_maxdays, opd.orders_products_filename, opd.orders_products_filename_2
...
#we can select our 128/192k version based on what we were sent
$dl_filename=$downloads->fields['orders_products_filename'];
if ($_GET['bitrate']=='192')
{	$dl_filename=$downloads->fields['orders_products_filename_2'];}
...
$zv_filesize = filesize(DIR_FS_DOWNLOAD . $dl_filename);
...
if (DOWNLOAD_BY_REDIRECT == 'true') {
  ...
  $download_link = str_replace(array('/','\\'),'_',$dl_filename);
  $link_create_status = symlink(DIR_FS_DOWNLOAD . $dl_filename, DIR_FS_DOWNLOAD_PUBLIC . $tempdir . '/' . $download_link);
...
// Call the exit function - otherwise it tries to include files that don't exist, throwing 500 errors
zen_exit();
?>
