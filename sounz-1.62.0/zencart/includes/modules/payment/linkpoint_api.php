 ...
 * @version $Id: linkpoint_api.php 5423 2006-12-28 10:59:25Z drbyte $
 */
...
// save errors which occur during checkout_payment validation phase but haven't been sent to gateway yet
  ...
  function before_process() {
    ...
    if (MODULE_PAYMENT_LINKPOINT_API_DEBUG != 'Off') {
      ...
      $myorder["webspace"]  = "false";  // for development only - not intended for production use
    }
    ...
    // handle errors
    if (trim($result) == '<r_approved>FAILURE</r_approved><r_error>Could not connect.</r_error>' || !is_array($result) || $result["r_approved"] != "APPROVED") {
      // do debug output
      $errorMessage = date('M-d-Y h:i:s') . "\n=================================\n\n" . ($mylphp->commError !='' ? $mylphp->commError . "\n\n" : '') . 'Response Code: ' . $result["r_approved"] . "\n\n" . 'Sending to Gateway: ' . $mylphp->sendData . "\n\n" . 'Result: ' . $mylphp->serverResponse . "\n\n" . 'CURL info: ' . print_r($mylphp->commInfo, true) . "\n";
      ...
      if (strstr(MODULE_PAYMENT_LINKPOINT_API_DEBUG, 'Log')) {
        ...
        $file = $this->_logDir . '/' . 'Linkpoint_Debug_' . $key . '.log';
        $fp = @fopen($file, 'a');
        @fwrite($fp, $errorMessage);
        @fclose($fp);
      }
      if (strstr(MODULE_PAYMENT_LINKPOINT_API_DEBUG, 'Email')) {
        zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, 'Linkpoint Debug Data', $errorMessage, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
      }
    }
...
//  Create arrays of responses for database storage
  ...
  function install() {
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Debug Mode', 'MODULE_PAYMENT_LINKPOINT_API_DEBUG', 'Off', 'Would you like to enable debug mode?  A complete detailed log of failed transactions may be emailed to the store owner.', '6', '0', 'zen_cfg_select_option(array(\'Off\', \'Log File\', \'Log and Email\'), ', now())");
