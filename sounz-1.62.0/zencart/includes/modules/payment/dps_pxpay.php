...
class dps_pxpay {
    ...
    function before_process() {
        ...
        if (array_key_exists('dps_done', $_GET)) {
            ...
            // comment
           // if (array_key_exists('result', $_GET) $this->_processPxPayResponse();
    ...
    global $order, $messageStack;
        ...
        // merchant reference
        $merchantRef = STORE_NAME . "-" . $customerId . "-" . date("YmdHis");

        // total order amount
        $total_amount = number_format($order->info['total'], 2, '.', '');

        // unique id
        $txnId = uniqid(rand(0, 999), false);

        $this->_createTrackingEntry($txnId, $merchantRef, $total_amount);
     ...
     * @param integer total_amount Order Total
     */
    function _createTrackingEntry($txnId, $merchantRef, $total_amount) {
    global $db;

        $sql = "insert into " . TABLE_DPS_PXPAY . " (txn_id, txn_type, merchant_ref, total_amount)
                values (:txnId, :txnType, :merchantRef, :total_amount)";
        ...
        $sql = $db->bindVars($sql, ':total_amount', $total_amount, 'currency');
    ...
    global $db;
        ...
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('Payment Text Title', 'MODULE_PAYMENT_DPS_PXPAY_TEXT_TITLE', 'Secure online Credit Card transaction using Visa or Mastercard via DPS Payment Express', 'Set the title that will be displayed to a customer.', '6', '3', '', '', now())");
    }
    ...
    function &_initCURL($query) {
        ...
        // SSLv3 protocol is disable due to security vulnerabilities
