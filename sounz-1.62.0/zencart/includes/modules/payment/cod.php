    ...
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_COD_ZONE > 0) ) {
       ...
       if ($order->delivery['country']['id']=='' )
		{
		 $check = $db->Execute("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_DPS_PXPAY_ZONE . "' order by zone_id");
		}
		else
		{
		...
		{
		#echo $order->info['shipping_module_code'];
		if ($order->info['shipping_module_code'] != 'storepickup_storepickup')
			{
			$this->enabled = false;
			}
		}
