...
?>
<p><strong>Downloads</strong><br/>You will have 3 days and 3 attempts to download the files.</p>

<hr/>
<table border="0" width="100%" cellspacing="0" cellpadding="5" id="downloads">
...
?>
  ...
  <?  if (  $downloads->fields['orders_products_filename_2'] != '')
		{?>
		<td class="centeredContent">( Standard 128kbps) <?php echo '<a href="' . zen_href_link(FILENAME_DOWNLOAD, 'order=' . $last_order . '&id=' . $downloads->fields['orders_products_download_id']) . '">' . zen_image_button(BUTTON_IMAGE_DOWNLOAD, BUTTON_DOWNLOAD_ALT) . '</a>'; ?><br/>
( High quality 192kbps) <?php echo '<a href="' . zen_href_link(FILENAME_DOWNLOAD, 'bitrate=192&order=' . $last_order . '&id=' . $downloads->fields['orders_products_download_id']) . '">' . zen_image_button(BUTTON_IMAGE_DOWNLOAD, BUTTON_DOWNLOAD_ALT) . '</a>'; ?>
</td>
    	<?}
	else
		{?>
