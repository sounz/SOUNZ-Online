...
// TABLES: address_format
  function zen_address_format($address_format_id, $address, $html, $boln, $eoln) {
    ...
    // check added by Liuba - as there is no validation for empty address fields
    // except for javascript one on address form
    if (trim($street) == '' && trim($city) == '') {
      $address_out = '<span class="alert">No address provided</span>';
    }
	else {
	    if ($html) {
	// HTML Mode
...
// validate customer matches session
  ...
  // Added by Liuba (Catalyst IT)  
  // returns a customer address array
  function zen_get_customer_address($customer_id, $address_id = 1) {
    global $db;
    $address_query = "select entry_firstname as firstname, entry_lastname as lastname,
                             entry_company as company, entry_street_address as street_address,
                             entry_suburb as suburb, entry_city as city, entry_postcode as postcode,
    ...
    $address_query = $db->bindVars($address_query, ':customersID', $customer_id, 'integer');
    $address_query = $db->bindVars($address_query, ':addressBookID', $address_id, 'integer');
	$address = $db->Execute($address_query);
