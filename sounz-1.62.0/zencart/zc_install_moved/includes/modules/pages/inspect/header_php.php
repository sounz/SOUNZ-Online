...
///////////////////////////////////
  ...
  //Check PgSQL version
  $pgsql_support = (function_exists( 'pg_connect' )) ? ON : OFF;
  $pgsql_version = (function_exists('pg_version')) ? @pg_version() : UNKNOWN;
  $pgsql_version = ($pgsql_version == '') ? UNKNOWN : $pgsql_version ;
  //if (is_object($db_test)) $mysql_qry=$db_test->get_server_info();
  $pgsql_ver_class = ($pgsql_version<'8.3') ? 'FAIL' : 'OK';
  $pgsql_ver_class = ($pgsql_version == UNKNOWN || $pgsql_version > '5.0') ? 'WARN' : $pgsql_ver_class;
  
  $status_check[] = array('Importance' => 'Critical', 'Title' => LABEL_POSTGRES_AVAILABLE, 'Status' => $pgsql_support, 'Class' => ($pgsql_support==ON) ? 'OK' : 'FAIL', 'HelpURL' =>ERROR_CODE_DB_NOTSUPPORTED, 'HelpLabel'=>ERROR_TEXT_DB_NOTSUPPORTED);
  if ($pgsql_version != UNKNOWN || ($pgsql_version == UNKNOWN && $advanced_mode)) $status_check[] = array('Importance' => 'Info', 'Title' => LABEL_MYSQL_VER, 'Status' => $pgsql_version, 'Class' => $pgsql_ver_class, 'HelpURL' =>($pgsql_version > '5.0' ? ERROR_CODE_DB_MYSQL5 : ERROR_CODE_DB_VER_UNKNOWN), 'HelpLabel'=>($pgsql_version > '5.0' ? ERROR_TEXT_DB_MYSQL5 : ERROR_TEXT_DB_VER_UNKNOWN) );
