...
<?php
  if ($set == 'payment') {
	$myQuery="select orders_status_id, orders_status_name from " . TABLE_ORDERS_STATUS . " WHERE ";
	if ($module->order_status != '')
		{$myQuery .="orders_status_id=" . $module->order_status . " and ";}
	$myQuery .= "language_id=" . $_SESSION['languages_id'];

    $orders_status_name = $db->Execute($myQuery);
?>
