...
// To use this newer query, remove the "1" from the following line ($products_query_raw1 becomes $products_query_raw )
    $products_query_raw1 =
      ...
      order by products_ordered DESC, pd.products_name";

  $products_query_numrows='';
  $products_split = new splitPageResults($_GET['page'], MAX_DISPLAY_SEARCH_RESULTS_REPORTS, $products_query_raw1, $products_query_numrows);

  $rows = 0;
  $products = $db->Execute($products_query_raw1);
  while (!$products->EOF) {
    $rows++;
