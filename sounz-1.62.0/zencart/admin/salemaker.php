...
// Create a string of all affected (sub-)categories
        ...
        if ($action == 'insert') {
          ...
          $_POST['sID'] = $db->Insert_ID(TABLE_SALEMAKER_SALES_SEQ);
      ...
      case 'copyconfirm':
        ...
        if (zen_not_null($newname)) {
          ...
          $sql_data_array = array('sale_id' => 'null',
            ...
            $sale_id = $db->Insert_ID(TABLE_SALEMAKER_SALES_SEQ);
            // update prices for products in sale
            zen_update_salemaker_product_prices($sale_id);
          }
        }

        zen_redirect(zen_href_link(FILENAME_SALEMAKER, 'page=' . $_GET['page'] . '&sID=' . $db->Insert_ID(TABLE_SALEMAKER_SALES_SEQ)));
        break;
      case 'deleteconfirm':
